...
# Load the shipment data (using requests to avoid SSL certificate issues on macOS)
shipments_url = "https://raw.githubusercontent.com/flyaflya/persuasive/main/shipments.csv"
shipments_df = pd.read_csv(
    StringIO(requests.get(shipments_url).text),
    parse_dates=['plannedShipDate', 'actualShipDate']
)

# Load product line data
product_line_url = "https://raw.githubusercontent.com/flyaflya/persuasive/main/productLine.csv"
product_line_df = pd.read_csv(StringIO(requests.get(product_line_url).text))
...
**Finding the dtype:** Use `shipments_df['actualShipDate'].dtype` (or `shipments_df.dtypes` for all columns). In this dataset the result is `datetime64[us]` (datetime with microsecond resolution).

**Why same type matters:** Comparing `actualShipDate` and `plannedShipDate` only works correctly when both are datetime types. Pandas then compares real dates (earlier vs later). If one were a string, you'd get lexicographic comparison (e.g. "2024-01-15" vs "2023-12-20") or a type error, so the `is_late` and `days_late` calculations would be wrong or fail.
:::

::: {.callout-tip}
## Your Answer: String vs Date Comparison

**As strings:** `"05-20-2024" > "04-11-2025"` is `True` because string comparison is character-by-character: `"05"` > `"04"`, so the first string is considered "greater" even though May 20, 2024 is earlier than April 11, 2025.

**As dates:** With proper datetime parsing, April 11, 2025 is *after* May 20, 2024, so the comparison would correctly show the later date as greater. Using dates ensures we get correct "on time" vs "late" logic instead of misleading string order.
:::

::: {.callout-tip}
## Your Answer: Debug This Code

**What's wrong:** Inside `.assign()`, `lateStatement` uses `shipments_df['is_late']` and the name `is_late`. At that moment, `is_late` does not exist on `shipments_df`—it is being created in the same `.assign()` call. So the code either raises a `KeyError` or, if `is_late` referred to something else, would use the wrong data.

**Fix:** Reference the *current* dataframe in the chain and the new column created in this same assign. Use a lambda so the expression is evaluated row-wise on the updated frame:  
`lateStatement=lambda df: df.apply(lambda row: "Darn Shipment is Late" if row['is_late'] else "Shipment is on Time", axis=1)`  
Or more simply, assign after the fact: add `lateStatement` in a second `.assign(lateStatement=lambda df: df['is_late'].map(lambda x: "Darn Shipment is Late" if x else "Shipment is on Time"))` so `is_late` already exists.
:::
...
**Difference:** `.query('is_late == True')` takes a string expression and evaluates it in the dataframe's namespace (column names as variables). `[df['is_late'] == True]` is boolean indexing: you build a boolean Series and use it to index. Both produce the same subset of rows.

**Readability:** `.query()` is often more readable for complex or multi-condition filters because the condition reads like a sentence (e.g. `query('days_late >= 5 and is_late')`). Boolean indexing is explicit and works well when the condition is already a variable (e.g. a mask). For simple checks like `is_late == True`, either is fine; `.query()` keeps the logic in one place when chaining.
:::

::: {.callout-tip}
## Your Answer: Additional Row Querying

Use a variable and reference it in `.query()` with `@` so pandas treats it as a Python variable, not a column name. Example for "at least 5 days late":
...
This keeps the threshold in one place and makes it easy to change (e.g. 5, 7, or 10 days) for different analyses.
:::
...
**Difference:** `.drop(columns=['quantity'])` removes the listed columns; everything else stays. `.filter([list of columns])` keeps only the columns you list and drops the rest.

**When to use which:** Use **drop** when you're removing a few columns from a wide table (e.g. one or two columns you don't need). Use **filter** when you want to work with a small, fixed set of columns (e.g. for a report or downstream step) and the rest are irrelevant. Drop is "remove these"; filter is "keep only these."
:::

::: {.callout-tip}
## Your Answer: Handling Missing Data

**Without `subset`:** `.dropna()` drops any row that has a missing value in *any* column. One NA in any column removes the whole row.

**With `subset`:** `.dropna(subset=['plannedShipDate', 'actualShipDate'])` drops only rows where at least one of those two columns is missing. Other columns can have NAs and the row is kept.

**Why be selective:** You may only care about completeness for the columns used in your analysis (e.g. dates for lateness). Dropping on all columns can remove rows that are otherwise usable and shrink your dataset more than needed. Specifying `subset` keeps rows that are complete where it matters.
:::
...
**ascending:** `ascending=False` sorts from largest to smallest (e.g. worst lateness first); `ascending=True` sorts from smallest to largest (default). So `ascending=False` on `days_late` puts the most late shipments at the top.

**Multiple columns:** Pass a list to `sort_values()`:  
`.sort_values(['is_late', 'days_late'], ascending=[False, False])`  
sorts first by `is_late` (True first), then by `days_late` (highest first) within each group. So late shipments appear first, and among those, the worst by days late are at the top.
:::

::: {.callout-tip}
## Your Answer: Index Management

**Why `.reset_index(drop=True)`:** Sorting does not change the index; row labels stay with their original row. So after sorting you get a non-sequential index (e.g. 42, 7, 103, …), which can break positional assumptions or cause duplicate index values. `.reset_index(drop=True)` replaces the index with a new integer range (0, 1, 2, …) and drops the old index, giving a clean, sequential index.

**What happens to the original index:** It is preserved as the index of the sorted dataframe until you reset it. If you don't reset, later operations (e.g. `.loc`, or "row 0") may not refer to the first row in display order, which is confusing and error-prone.
:::
...
In Python and pandas, `True` is treated as 1 and `False` as 0 in numeric contexts. So `.sum()` on a boolean Series counts how many values are `True` (the "late" shipments). That makes `sum()` a direct count of late shipments, and `.mean()` is the proportion of True values (e.g. the fraction or percentage of late shipments). No need to convert to int first.
:::
...
**Why `how='left'`:** A left join keeps every row from the left table (shipments) and attaches matching rows from the right (product line). So we never drop a shipment just because it has no (or a bad) product-line match—we preserve all shipment records and get NaNs where there's no match. That avoids unintentionally losing data when the right table is a lookup.

**Checking for lost rows:** Compare row counts before and after the merge: `len(clean_shipments)` vs `len(shipments_with_category)`. With `how='left'` and a single match per key on the right, the count should stay the same. In this dataset we have 4,000 rows before and 4,000 after, so no shipments were lost.
:::

::: {.callout-tip}
## Your Answer: Key Column Matching

If there are duplicate `partID` values in `product_line_df`, the merge will duplicate left rows: each shipment row will match every matching product-line row, so one shipment can become many rows (one per duplicate). That inflates counts and can double-count in later aggregations. In this dataset, `product_line_df` has no duplicate `partID`s (each part maps to one product line and category), so the left join yields one row per shipment. Always check with `product_line_df['partID'].duplicated().any()` (or `.sum()`) before merging.
:::
...
**What `.groupby('prodCategory')` does:** It splits the dataframe into one group per distinct value of `prodCategory`. Each group is a subset of rows that share that category. Pandas doesn't show the raw groups; it waits for an aggregation (or other operation) to summarize or transform each group.

**Why we need `.agg()` (or similar):** After grouping, you have a GroupBy object, not a single dataframe. You must specify how to combine each group—e.g. `.agg()` to get summary stats per group. Without an aggregation (or `.apply()`/`.transform()`), you don't get a concrete result. So the pattern is: split by group, then apply a function (count, sum, mean, etc.) to each group to get one row per category.
:::

::: {.callout-tip}
## Your Answer: Multi-Level Grouping

**Grouping by `['shipID', 'prodCategory']`:** Each row is one shipment–category combination. So if a shipment has line items in both "Electronics" and "Home", you get two rows for that shipID (one per category). This answers: "For each shipment, what is the lateness status per product category?"—e.g. "In this order, were Electronics on time but Home late?"

**Vs grouping by `'prodCategory'` alone:** That gives one row per category across *all* shipments (e.g. "Electronics: 80% on time overall"). So `prodCategory` alone = category-level service level; `shipID` + `prodCategory` = shipment-level detail by category, which you need to see which *orders* touch multiple categories and how each category performed within that order.
:::
...
**Business question:** "How often do orders (shipments) include products from more than one product category?"—i.e. cross-category shopping behavior.

**Grouping by `['shipID', 'prodCategory']` vs `'prodCategory'`:** Grouping by both produces one row per shipment–category pair, so we can count how many distinct categories each shipID has (e.g. with `groupby('shipID')['prodCategory'].nunique()`). Grouping by category alone only gives overall category totals, not per-shipment breakdown, so we couldn't compute "this shipment had 2 categories."

**Insights for ZappTech:** The percentage of multi-category shipments tells management how often customers bundle across categories (e.g. Electronics + Home). That informs bundling, promotions, fulfillment, and inventory placement. High multi-category share may justify cross-sell strategies or combined shipping; low share may suggest need for better discovery or offers across categories.
:::
